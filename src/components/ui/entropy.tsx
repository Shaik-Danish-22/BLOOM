...
export function Entropy({ className = "", size = 400 }: EntropyProps) {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      draw(ctx: CanvasRenderingContext2D) {
        const alpha = this.order ? 0.7 - this.influence * 0.4 : 0.7
  ...
  return (
    <div className={`relative bg-transparent ${className}`} style={{ width: size, height: size }}>
      <canvas ref={canvasRef} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-30" />
    </div>
  )
}